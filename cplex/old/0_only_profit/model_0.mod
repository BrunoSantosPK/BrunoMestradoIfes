 ...
 * OPL 20.1.0.0 Model
 * Author: bruno
 * Creation Date: 1 de mar de 2024 at 16:23:26
 *********************************************/
...
float PAC = ...; // valor em R$ / kg do adubo utilizado nas plantações
...
dvar int+ PC[i in I][j in J]; // quantidade em kg do cultivo produzido j que é utilizado para consumo dos moradores no mes i
dvar int+ RC[i in I][j in J][k in K]; // quantidade em kg do cultivo produzido j que é utilizado para alimentacao do animal k no mes i
dvar int+ FC[i in I]; // quantidade em kg de fertilizante comprado no mes i
dvar int+ CA[i in I][k in K]; // quantidade de lotes do animal k comprados para criação no mes i
...
dvar int+ VP[i in I][k in K]; // quantidade (unidade especıifica) de produto gerado pelo animal k vendido no mes i
dvar int+ AP[i in I][k in K]; // quantidade (unidade especıifica) de produto gerado pelo animal k consumido no mes i
dvar int+ CP_[i in I][k in K]; // quantidade (unidade especıifica) de produto do animal k comprado no mes i
...
maximize (
	...
	// geral pela compra de fertilizante (FC, kg) e o preço (PAC, R$/kg)
	- sum(i in I) FC[i] * PAC
...
subject to {
	...
	forall(i in I)
	  LimiteLotesExistentes: sum(l in L, j in J) LC[l][i][j] + sum(l in L, k in K) LA[l][i][k] <= mini_batchs;
	...
	forall(i in I, l in L)
	  MinilotesSemOcupacaoSimultanea: sum(j in J) LC[l][i][j] + sum(k in K) LA[l][i][k] <= 1;
	  
	// Quando um minilote for ocupado por uma plantacao, ele deve permanecer ocupado ate o
	// tempo em que essa plantacao sera colhida, ou seja, durante todo o tempo de
	// crescimento da planta (TCC); importante comentar que no o i-1 aparece porque
	// no mes final nao precia estar ocupado (exemplo: uma planta com tempo de 3 meses que
	// foi plantada no mes 2 precisa ocupar o mes 2, mes 3 e mes 4, no mes 5 ela e colhida
	// e libera o minilote para outra utilizacao)
	forall(l in L, j in J, i in I: i >= TCC[j])
	  MiniloteOcupadoDuranteCrescimento: sum(a in (i-TCC[j] .. i-1)) LC[l][a][j] >= LC[l][i - TCC[j]][j] * TCC[j];
	...
	forall(i in I)
	  NecessidadeDeAdubacaoDasPlantas: sum(l in L, j in J) (LC[l][i][j] * RAC[j] * area_mini_batch) <= FC[i]
	...
	forall(i in I, j in J)
	  SegurancaAlimentarPlantacoes: P * NAC[j] == CC[i][j] + PC[i][j];
	  
	forall(i in I, k in K)
	  SegurancaAlimentarCarneAnimal: P * NAA[k] == PA[i][k] + AA[i][k] * PYA[k] * AB[k];
	  
	forall(i in I, k in K)
	  SegurancaAlimentarProdutoAnimal: P * NAP[k] == CP_[i][k] + AP[i][k];
}
