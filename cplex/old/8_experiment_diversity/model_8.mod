...
dexpr float food_security = (
	// Estas parcelas sao responsaveis por garantir uma maximizacao
	// da seguranca alimentar, ou seja, que os membros da familia
	// tenham alimentos em abundancia.
	(sum(i in I, j in J) (aEC[i][j] - NAC[j] * P) - sum(i in I, j in J) PC[i][j]) / sum(j in J) (NAC[j] * months)
	
	+ (sum(i in I, k in K) (AP[i][k] - P * NAP[k]) - sum(i in I, k in K) AP[i][k]) / sum(k in K) (NAP[k] * months)
	
	+ (sum(i in I, k in K) (AA[i][k] * AB[k] * PYA[k] - P * NAA[k]) - sum(i in I, k in K) (AA[i][k] * AB[k] * PYA[k])) / sum(k in K) (NAA[k] * months)
);

dexpr float gini_diversity = (
	...
	// no tempo i
	sum(i in I, a in J, b in J) abs(aXT[i][a] - aXT[i][b]) / sum(i in I, a in J) (2 * n_plants * aXT[i][a])
);


// Funcao objetivo
minimize gini_diversity;
...
subject to {
  ...
  forall(k in K, i in I: i >= TCA[k])
    SegurancaAlimentarCarneAnimal: AA[i][k] * AB[k] * PYA[k] >= NAA[k];
...
execute {
  /*for(var m in I) {
  	...
  	writeln("Lotes com animais: ", animals[m]);
  	writeln("Lotes com plantas: ", cultives[m]);
  	writeln("");
  }*/
  ...
  writeln("Diversidade: ", gini_diversity);
}
